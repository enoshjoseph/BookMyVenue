...
import { getVenueById, updateVenue, uploadVenueImages } from "@/services/venueService";
  ...
  const loadVenueDetails = async () => {
    try {
      ...
      if (data) {
        ...
        setIsAdvanceRequired(data.isAdvanceRequired || false);
        setAdvancePercentage(data.advancePercentage?.toString() || "");
        setBalanceDueDays(data.balanceDueDaysBeforeEvent?.toString() || "");
        setIsCancellationAllowed(data.isCancellationAllowed || false);
        setCancellationDeadlineDays(data.cancellationDeadlineDays?.toString() || "");
      }
    } catch {
    ...
    try {
      ...
      const msg =
        err && typeof err === "object" && "response" in err
          ? (err as { response?: { data?: { message?: string } } }).response?.data?.message || "Failed to update venue."
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1">
        <header className="flex h-16 items-center gap-4 border-b border-gray-200 bg-white px-6 lg:px-8">
          ...
          <h1 className="text-lg font-semibold text-gray-900">Edit Venue</h1>
        </header>
